...
function openCurseModal() {
    const modal = new bootstrap.Modal(document.getElementById('curseModal'));
    modal.show();
}
...
function addNewCurse() {
    ...
    const modal = bootstrap.Modal.getInstance(document.getElementById('curseModal'));
    modal.hide();
}
...
function exportCharacterSheet() {
    ...
    const characterData = {
        ...
        curses: Array.from(document.querySelectorAll('.curse-card')).map(curse => {
            ...
            const descriptionElement = curse.querySelector('p:last-of-type');
...
function calculateDerivedStats() {
    ...
    document.getElementById('current-hp').textContent = totalHP;
    updateHPBar();
    
    document.getElementById('max-pa').textContent = totalPA;
    document.getElementById('current-pa').textContent = totalPA;
    updatePABar();
    ...
    // Adicionar maldições da trilha baseadas no nível
    if (currentPath && currentPath.curses) {
        currentPath.curses.forEach(curse => {
            if (level >= curse.level && !document.getElementById(`curse-${curse.name.replace(/\s+/g, '-')}`)) {
...
const predefinedEquipment = [
    ...
    { name: "AK-47", damage: "3d8 (média distância)." },
...
const predefinedPaths = [
    ...
    },
    {
        ...
        summary: "Controla criaturas paranormais através de voz, olhar e presença, manipulando-as com técnicas que variam de persuação a dominação absoluta.",
        curses: [
            {
                level: 5,
    ...
    pathInput.addEventListener('input', function () {
        ...
        if (matches.length > 0) {
            matches.forEach(path => {
                ...
                summarySpan.className = 'suggestion-damage';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (lockBtn) lockBtn.addEventListener('click', disableEditing);

    const attributesAndLevel = document.querySelectorAll(
        '.stats-grid div:nth-child(6) .stat-value span, ' +
    ...
    if (equipmentModal) {
        equipmentModal.addEventListener('shown.bs.modal', function() {
            const equipmentName = document.getElementById('equipment-name');
            if (equipmentName) equipmentName.focus();
        });
    }
